"""
jobs.de scraper — general German job portal with broad coverage.

jobs.de aggregates listings from many German companies and other portals.
...
JOBS_DE_BASE = "https://www.jobs.de"
JOBS_DE_SEARCH = f"{JOBS_DE_BASE}/stellenangebote"


async def scrape_jobs_de(
    positions: List[str],
    locations: Optional[List[str]] = None,
    max_results: int = 50,
    headless: bool = True,
) -> List[JobSchema]:
    """Scrape jobs.de job listings."""
    ...
    async with httpx.AsyncClient(
        ...
        for position in positions:
            for location in locations:
                logger.info(f"[Jobs.de] Scraping '{position}' in '{location}'")
                try:
                    jobs = await _fetch_jobs(client, position, location, max_results)
                    all_jobs.extend(jobs)
                except Exception as e:
                    logger.warning(f"[Jobs.de] HTTP failed ({e}), trying Playwright...")
                    try:
                        jobs = await _scrape_playwright(position, location, max_results, headless)
                        all_jobs.extend(jobs)
                    except Exception as e2:
                        logger.error(f"[Jobs.de] All methods failed: {e2}")
                await asyncio.sleep(1)

    logger.info(f"[Jobs.de] Total collected: {len(all_jobs)} jobs")
    return all_jobs
...
async def _fetch_jobs(
    ...
    while len(jobs) < max_results:
        ...
        resp = await client.get(JOBS_DE_SEARCH, params=params)
        ...
        cards = (
            ...
            or soup.find_all("div", class_=lambda c: c and "stellenangebot" in (c or "").lower())
...
def _parse_card(card) -> Optional[JobSchema]:
    try:
        ...
        job_url = href if href.startswith("http") else f"{JOBS_DE_BASE}{href}" if href else ""
        if not job_url:
            return None
        ...
        logger.debug(f"[Jobs.de] Card parse error: {e}")
        return None
...
async def _scrape_playwright(
    ...
    async with async_playwright() as pw:
        ...
        await page.goto(
            f"{JOBS_DE_SEARCH}?q={quote_plus(position)}&l={quote_plus(location)}",
